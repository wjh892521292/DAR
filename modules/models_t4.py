...
class FrozenCLIPEmbedder(nn.Module):
    ...
    def __init__(self, version="openai/clip-vit-large-patch14", device="cuda", max_length=77, pool=True):
        ...
        version = "/root/hy58/caijin/huggingface_hub/openai_clip/"
...
class BasicUpdateBlockDepth(nn.Module):
    def __init__(self, hidden_dim=128, q_dim=128, memory_dim=1536, context_dim=320, patch_hws_list=None, channels_in=128*8, channels_out=128, args=None):
        ...
        self.transdecoder = TransformerDecoder(dim=q_dim, mid_dim=q_dim*2, memory_dim=memory_dim, num_heads=8, num_layers=13, use_long_skip=True)
        ...
        # patch_hws_list=[(1,2),(2,3),(3,4),(4,5),(6,8),(8,10),(12,15),(16,20)]

        if patch_hws_list != None:
            self.patch_hws_list = patch_hws_list


        self.project = Projection(1536, q_dim)
    ...
    def forward(self, depth, context, input_hidden, seq_len, depth_num, min_depth, max_depth, patch_list):
        ...
        # c_list = []
        # for out_shape in patch_list:
        #     new_c =  F.adaptive_max_pool2d(input_hidden, out_shape)
        #     c_list.append(new_c)
        ...
        for i in range(seq_len):
            ...
            if i != 0:
                ...
                # bin_prompt = self.encoder(current_depths.detach())
                # bin_prompt = torch.flatten(bin_prompt, 2).transpose(1, 2)

                # next_tokens_c = self.qformer(next_tokens, bin_prompt, tgt_mask=None)
            ...
            # print(i)
            # debug_mode('shape1', token_seq)
            # debug_mode('shape2', memory)

            output = self.transdecoder(token_seq, memory, tgt_mask=tgt_mask)
            output = output[:, (-ph*pw):, :]
            # debug_mode('shape2', q_output_hidden)
            # debug_mode('shape3', next_token_map)

          
            # print(output.shape, ph, pw)
            output_maps = output[:, :, :].transpose(1, 2).view(b, -1, ph, pw) # b, 1024, ph, pw
            next_maps = output_maps

            # debug_mode('shape3', output_maps)
            ...
            right_max = torch.ones_like(label_target_bin_right) * (depth_num)
            ...
            # target_bin_left = torch.gather(bin_edges, 1, torch.clamp_min(label_target_bin_left-1, left_min))
            # target_bin_right = torch.gather(bin_edges, 1, torch.clamp_max(label_target_bin_right+1, right_max))
