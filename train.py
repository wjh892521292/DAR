...
# from models.model2 import EcoDepth as MyModel
from models.model import DAR as MyModel
...
def train(train_loader, val_loader, model, model_without_ddp,  criterion_d, log_txt, optimizer, device, curr_epoch, args, tboard_writer):    
    ...
    for batch_idx, batch in enumerate(train_loader):      
        ...
        if args.encoder == 'VIT-16':
            multiple = 64
        elif args.encoder == 'VIT-14':
            multiple = 56

        desired_multiple_of = multiple
        ...
        # print('old:', old_w, old_h, '----new:', new_w, new_h)
        ...
        for each_pred in pred_d:
            new_pred_d = each_pred[:, :, :old_h, :old_w]
            # pred_d = new_pred_d

            loss_d += criterion_d(new_pred_d.squeeze(dim=1), depth_gt)
        pred_vis = preds['pred_d']
        pred_d = pred_d[-1]
            ...
            if batch_idx % args.log_images_freq == 0: 
                ...
                for i in range(num_log_images):
                    try:
                        ...
                        for j in range(len(pred_vis)):
                            pred_vis_d = pred_vis[j]
                            tboard_writer.add_image('depth_est/image/{}_{}'.format(i, j), colorize_depth(pred_vis_d[i].detach().cpu().numpy().squeeze()[::2,::2]).transpose((2,0,1)), global_step)
        ...
        if args.rank == 0 and global_step % args.model_save_freq == 0:
            ...
            global best_rmse, best_abs_rel, best_d1, log_dir
            ...
            if results_dict['d1'] > best_d1:
                model_save_name = f"best_d1_{results_dict['d1']:.4f}.ckpt"
                print(f"Saving best model as {model_save_name} in {log_dir}")
                if best_d1 > 0:
                    os.remove(os.path.join(log_dir, f"best_d1_{best_d1:.4f}.ckpt"))
                best_d1 = results_dict['d1']
                torch.save(
                    {
                        'model': model_without_ddp.state_dict(),
                        'epoch': curr_epoch,
...
def main():
    ...
    if args.rank == 0:
        ...
        os.system(command)
        command = "mkdir -p "+os.path.join(file_dir, 'modules') + '&& cp modules/*.py ' + os.path.join(file_dir, 'modules')
        os.system(command)
    else:
        log_txt = None
        log_dir = None  
    ...
    start_ep = 1
    global global_step, best_rmse, best_abs_rel, best_d1
    global_step = 0
    best_rmse = 1000
    best_abs_rel = 1000
    best_d1 = 0
    ...
    if args.resume_from: # if training crashed in b/w then resume from the last checkpoint.
        ...
        results_dict['rmse'] = 0.24
        results_dict['abs_rel'] = 0.0646
        results_dict['d1'] = 0.97
